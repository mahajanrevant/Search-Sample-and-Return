...
def rock_thresh(img, rock_thresh=(130, 180, 100,170,0,30)):
    # Create an array of zeros same xy size as img, but single channel
    color_select = np.zeros_like(img[:,:,0])
    # Require that each pixel be above all three threshold values in RGB
    # above_thresh will now contain a boolean array with "True"
...
def perception_step(Rover):
    ...
    obs_threshed = obstacle_thresh(warped) * mask
    ...
    xpix,ypix = rover_coords(color_threshed)
    obsxpix, obsypix = rover_coords(obs_threshed)    
    ...
    obsx_world, obsy_world = pix_to_world(obsxpix, obsypix, xpos, ypos, yaw, world_size, scale)    
    ...
    if rock_threshed.any():
        ...
        nearest = np.argmin(rock_dist)
        rock_x = rock_x_world[nearest]
        rock_y = rock_y_world[nearest]
